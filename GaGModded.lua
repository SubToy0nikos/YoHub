...
if can_get_free_seeds then
    ...
    if not fruits then
        return local_player:Kick("Found remote to get free seeds but couldnt find fruits report this to @yonikosyt on discord!")
    end
...
if can_get_free_gears then
    ...
    if not gears then
        return local_player:Kick("Found remote to get free gears but couldnt find gears report this to @yonikosyt on discord!")
    end
...
local window = library:CreateWindow({
    Title = "YoHub | Made By YOnikosYT x BlushyFemboy",
    Center = true,
    AutoShow = true,
    TabPadding = 8,
    MenuFadeTime = 0.4
...
save_manager:SetFolder('Sybau Skid')
